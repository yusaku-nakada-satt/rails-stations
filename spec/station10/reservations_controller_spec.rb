...
RSpec.describe ReservationsController, type: :controller do
  ...
  describe 'Station10 GET /movies/:movie_id/schedules/:schedule_id/reservations/new' do
    ...
    let(:success_request) do
      get :new, params: { movie_id: movie.id, schedule_id: schedule.id, date:, sheet_id: sheets.first.id },
                session: {}
    end
    ...
    let(:no_sheet_request) do
      get :new, params: { movie_id: movie.id, schedule_id: schedule.id, date: }, session: {}
    end
  ...
  describe 'Station10 POST /reservation/' do
    ...
    let(:success_request) do
      post :create,
           params: { reservation: { name: 'TEST_NAME', email: 'test@test.com', date:, sheet_id: sheets.first.id, schedule_id: schedule.id, movie_id: movie.id } }, session: {}
    end
    ...
    it 'DBのunique制約にかかったときにリダイレクトテスト' do
      # 同じ日付の同じ映画の座席を予約してみる
      create(:reservation, { sheet_id: sheets.first.id, schedule_id: schedule.id, date: })
      success_request
